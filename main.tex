...
Zum Verständnis des Kapitel \nameref{sec:methoden-und-techniken} sind Grundkenntnisse der Informatik nötig. Diese werden in diesem Kapitel erarbeitet. Wie am Anfang bereits erwähnt diente, sofern keine andere Quelle angegeben wurde, das Buch ``Hacking - The Art of Exploitation''~\cite{erickson2008hacking} als Hauptquelle dieses Kapitels.
...
In den meisten Programmiersprachen gibt es Möglichkeiten, Text in die Kommandozeile zu drucken. Um variablen Input zu drucken, werden in C Format Specifiers gebraucht. Format Specifiers geben an, welcher Datentyp erwartet werden soll. Der Format Specifier \texttt{\%d} für Ganzzahlen kann auf diese Weise eingesetzt werden: 
\begin{lstlisting}
	int main() {
		int zahl = 42;
...
\subsubsection{Allgemeine Funktionsweise}
Voraussetzung dieser Attacke ist ein Programm, welches eine Angabefunktion wie \texttt{prinf()} verwendet, um vom Benutzer gegebenen Input zu drucken. Dabei wird der Input aber nicht explizit als String, sondern direkt gedruckt. Der Programmteil unten zeigt den Unterschied.
\begin{lstlisting}
	printf("%s", text); // korrekte Art
	printf(text);  // falsche Art
\end{lstlisting}

Die falsche Implementation erlaubt dem Angreifer die Benutzung von eigenen Format Specifiers. Zum Verständnis der Attacke muss die Speichersegmentierung bei einem \texttt{printf()}-Funktionsaufruf bekannt sein. 

\paragraph{Speichersegmentierung bei printf()}
Bei Funktionsaufruf einer \texttt{printf()}-Funktion wird eine Rückkehradresse zur Ursprungsfunktion, die Speicheradresse des zu druckenden Textes und die Argumente der \texttt{printf()}-Funktion in dieser Reihenfolge auf den Stack gelegt. Die Argumente stehen jeweils direkt hintereinander. Je nach Datentyp der variablen Argumente werden die Werte direkt, also nicht als Referenzierung durch Speicheradressen, auf den Stack gelegt. Beim Format Specifier \texttt{\%x}, der Hexadezimale Zahlen in den Text einbettet, ist dies der Fall. 


Die Benutzung eigener Format Specifier kann auf zwei Arten ausgenutzt werden:
\begin{enumerate}
	\item \textbf{Speicherlesen}: Format Specifier wie \texttt{\%x} erwarten Argumente. Werden keine Argumente angegeben, lesen sie die Daten an der Stelle aus, an der die Argumente erwarten werden. Durch Wiederholung von \texttt{\%x} lassen sich also ab der Stelle, bei der die \texttt{printf()}-Argumente im \gls{ram} erwartet werden, Daten in die Kommandozeile drucken. Dies erlaubt möglicherweise das vom Programmierer unerwünschte Auslesen von Daten.
	\item \textbf{Speichermanipulation}: Bedeutend schlimmer ist die unerwünschte Veränderung von Daten im \gls{ram}. Dazu kann der Format Specifier \texttt{\%n} genutzt werden. Er schreibt die bisherige Anzahl gedruckter Zeichen an eine, als Argument angegebene Speicheradresse. Das Beispiel zeigt die Funktionalität von \textit{\%n}:
	\begin{lstlisting}
		int main() {
			int count;
	...
	Der Referenzoperator \texttt{\&} referenziert eine Variable, gibt also ihre Speicheradresse zurück.\\ Diese Attacke wird speziell kritisch, wenn der gedruckte Text weiter unten im Stack gespeichert wird und sich selbst verwenden kann. Dadurch können gezielt Speicheradressen manipuliert und durch geschickte Wahl des Input mit eigenen Daten gefüllt werden. Das Beisiel in Kapitel \ref{subsubsec:bspprintf} demonstriert das Schreiben einer eigenen Speicheradresse an einen bestimmten Punkt im \gls{ram}. Mithilfe eines Shellcodes kann dies dieselben Konsequenzen wie eine \ref{subsec:buffer-overflow}-Attacke von sich tragen.
\end{enumerate}
...
Das Programm \textbf{fmt\_vuln.c} druckt einen vom Benutzer als Kommandozeilenargument gegebenen Input in die Kommandozeile. Die Sicherheitslücke besteht darin, dass das Programm den Text nicht explizit als String druckt, sondern direkt. Die zwei wichtigen Zeilen sind unten abgedruckt.
